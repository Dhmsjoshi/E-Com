...
//    }
    ...
    @Operation(summary = "Refresh Access Token", description = "Generates a new Access Token using a valid Refresh Token from cookies.")
    @ApiResponse(responseCode = "200", description = "Token refreshed successfully")
    ...
    public ResponseEntity<Map<String, String>> refresh(
        ...
        Map<String, String> tokens = authorizationService.refreshAccessToken(refreshToken);
        return ResponseEntity.ok(tokens);
    }

    @Operation(summary = "Logout user", description = "Revokes the current refresh token and clears the session.")
    @ApiResponse(responseCode = "204", description = "Logged out successfully")
    @PostMapping("/logout")
    public ResponseEntity<Void> logout(@CookieValue(name = "refreshToken", required = false) String refreshToken) {
        if (refreshToken != null) {
            authorizationService.removeByToken(refreshToken);
        }
